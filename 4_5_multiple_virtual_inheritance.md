...
При преобразовании указателя или ссылку наследника к указателю или ссылке базового класса, то указатель сдвинется с начала объекта(адрес m)  на адрес d.

Если у родителей есть метод f(), то будет перегрузка между ними. Аргументы могут быть разными. Если методы идентичны, то при добавлении их в область видимости СЕ не будет, но при их вызове будет СЕ(так же и с полями работают)
...
```
Но никто не запрещает обратится напрямую к через родиля:
```
Son son;
son.Mom::gg
```
...
5. Cast от Grandma к Son тоже не определен.
...
А если попробуем использовать Ded вместот Mom?
```
void (Ded::*p)() = &f;
(s.*p)();
```
На самом деле указатель на метод еще хранит в себе сдвиг, по которому вызывается метод.
Указатель на метод: |ptr to method|shift|
                    |-------------|-----|
Изначально указатель `Ded::*p` хранит сдвиг 0. Но так как мы вызывается от сына то нужно произвести сдвиг.
```
void (Ded::*p)() = &f; // shift = 0
void (Son::*ps)() = &fs; // shift становится  не 0
(s.*ps)();
```
...
Итого: (ptr = 8б) + (m = 4б) + (ptr = 8б) + (d = 4б) + (s = 4б) + (g = 4б) + (выравнивание = 4б) = 36б
