...
Поговорим о реализации итератора в контейнере. Вернемся к проекту вектора и реализуем внутри него итератор. Что есть у итератора?
...
Добавим поддержку `reverse_iterator`. Это обертнка которая позволяет итерироваться по контейнеру в обратном порядке.  
...
v.push_back(1);
```
Да, итераторы тоже инвалидируются, а сделано это потому чтобы итераторы были такими же быстрыми как указатели. За счет этого итератор на `std::deque` инвалидируется тоже.
...
```
Это приведет к `UB` ведь мы будем пистаь вне контейнера. Для решения этой проблемы изобрели `std::back_insert_iterator`. Это `OutputIterator` который гарантирует отсутствие записи в не свою память. Ну и решение нашей проблемы:
```
int main() {
  int a[] = {1, 2, 3, 4, 5};
  std::vector<int> v;
...
```
Обычно `back_insert_iterator` писать грамоздко поэтому существует функция для более короткой записи, реализуем:
```
template <typename Container>
