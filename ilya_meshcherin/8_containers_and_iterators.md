...
Немного `UB`!
Многие уже понимают, что данный ниже код дает нам `UB`:
```cpp 
...
Под капотом, как ранее говорилось, лежат итераторы. Т.е. данный код эквивалентен такому:
```cpp 
std::vector<int> v = {1, 2, 3, 4, 5};
...
Однако, код с `range-based for` будет работать корректно с `UB`, а код с итераторами уйдет в `Segmentation fault`. Почему?

`Range-based for` сохраняет для оптимизации `end`, чтобы каждый раз не вызывать функцию. А код с итераторами будет пересчитывать конец вектора после его реаллокации и за счет этого цикл никогда не достигнет конца.
